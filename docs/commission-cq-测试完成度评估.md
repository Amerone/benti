...
| R12 | CQ 草案可维护、查询、修改 | 保存草案、查询草案、修改状态为 `reviewed` | `/cq-engine/drafts`、`PATCH /drafts/{draft_id}`；页面有生成保存和 reviewed 操作 | 部分通过 |
| R13 | 页面可以维护、查询、修改本体 | 本体页面能加载和查询，能维护/修改正式本体结构 | 当前有 ontology load/list/subjects 能力；没有正式本体编辑、版本发布、回滚 API | 未满足 |
| R14 | 新建任意委托单 | 用表单/API 创建非 `CO-2024-001` 委托单 | OpenAPI 无 `POST /commission/orders` | 未满足 |
| R15 | 任意试验任务录入新测试数据 | 用 API/页面录入新的 commission 测试数据记录并判定 | OpenAPI 无 `POST /commission/data-records` | 未满足 |
| R16 | CQ 草案发布/导出 | reviewed 草案发布为正式 Turtle/CQ/规则资产 | OpenAPI 无 `/cq-engine/drafts/{draft_id}/publish` | 未满足 |
...
- 草案正文在线编辑
- reviewed 草案发布/导出到正式资产
- 发布后的版本和回滚记录

### 产品化 CRUD 完成度：低到中

当前更接近“固定剧本 + 可验证工程样机”，不是完整业务系统。

主要缺口：

- 不能创建任意委托单。
- 不能新增任意试验项目。
- 不能录入任意 commission 测试数据。
- 不能在线修改正式本体结构。
- 不能发布/回滚正式本体版本。
...
如果验收目标是“作为可配置、可维护、可录入的业务系统上线试点”，当前系统只能判定为演示级完成，仍需补齐通用委托单 CRUD、测试数据录入、正式本体维护和草案发布/导出。

建议下一轮按 TDD 补齐三个高优先级缺口：
